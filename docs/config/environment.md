...
| `ALFRED_MOCK_SCRIPTS` | _(none)_ | Path to a module default-exporting MockProvider `Script[]`. When set, `alfred` and `alfred run` swap in the scripted offline provider — zero API calls, no key needed — while the engine, tools, permission stack, verify gate and signed ledger all run for real. Powers `bun run demo`; the same record/replay mechanism `alfred eval` uses. | `src/index.ts` |
