/**
 * Scripted model for the offline demo — drives `alfred run` end-to-end with
 * ZERO API calls. The engine, tools, permission stack, verify gate and signed
 * ledger are all real; only the LLM responses below are recorded. This is the
 * same record/replay mechanism `alfred eval` uses to gate regressions in CI.
 *
 * Usage (from this directory):
 *   ALFRED_MOCK_SCRIPTS=./scripts.ts \
 *     bun ../../src/index.ts run --verify "bun test add.test.ts" --max-features 1
 */
import { textResponse, toolUseResponse, type Script } from "../../src/providers/mock.ts";
...
const demoScript: Script = (messages) => {
  ...
  return sawToolResult(messages)
    ...
    : toolUseResponse("file_write", { path: "add.ts", content: ADD_TS }, { text: "Writing add.ts…" });
};

export default [demoScript] satisfies Script[];
